...
function builder<T extends Clients>(
  ...
  return {
    ...
    async set(key, value, ttl) {
      if (!isCacheable(value)) throw new Error(`"${value}" is not a cacheable value`);
    ...
    mget: (...args) =>
      redisCache
        .mGet(args)
        .then(x =>
          x.map(x => (x === null || x === undefined ? undefined : (JSON.parse(x) as unknown))),
        ),
    async mdel(...args) {
...
export function redisClusterInsStore(
  ...
  const reset = async () => {
    await Promise.all(redisCache.getMasters().map(node => node.client.flushDb()));
  };

  const keys = async (pattern: string) =>
    (await Promise.all(redisCache.getMasters().map(node => node.client.keys(pattern)))).flat();

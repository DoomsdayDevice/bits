...
import { FilterOperation, NestedFindManyOpts } from "@bits/backend";
...
export class NestedQuery<T extends ObjectLiteral> {
  ...
  private applyJoins(
    ...
    allRels.forEach((rel, index) => {
      ...
      const path = `${this.getMappedAlias(
        _.camelCase(parentAlias),
        aliasMap
      )}.${childName}`;
  ...
  private parseFindOperator(
    ...
    if (opType === "in") {
      if (!operator.value.length) query.andWhere("FALSE");
        ...
        query.andWhere(`${path} IN (:...${valueName})`, {
          [valueName]: operator.value,
        });
      ...
      if (subOpType && subOpType === "isNull") {
        ...
        query.andWhere(`${path} NOT IN (:...${valueName})`, {
          [valueName]: subOperator.value,
        });
      } else {
        query.andWhere(`${path} != :${valueName}`, {
          [valueName]: operator.value,
        });
      }
  ...
  private getRelsFromFilter(filter: any): string[] {
    const { oneToManyRelations, manyToOneRelations, manyToManyRelations } =
      getRelations(this.Entity);
    const rels = [
      ...oneToManyRelations,
      ...manyToOneRelations,
      ...manyToManyRelations,

...
export class ValueReflector extends Reflector {
  set<DTO, Data>(DTOClass: Type<DTO>, data: Data): void {
    this.defineMetadata(data, DTOClass);
  }

  get<DTO, Data>(DTOClass: Type<DTO>, includeParents = false): MetaValue<Data> {
    return this.getMetadata(DTOClass, includeParents);
  }

  isDefined<DTO>(DTOClass: Type<DTO>): boolean {
    return this.get(DTOClass) !== undefined;
  }

  memoize<DTO, Data>(DTOClass: Type<DTO>, fn: () => Data): Data {
...
export class ArrayReflector extends Reflector {
  append<DTO, Data>(DTOClass: Type<DTO>, data: Data): void {
...
export class MapReflector<K = string> extends Reflector {
  set<DTO, Data>(DTOClass: Type<DTO>, key: K, value: Data): void {
    const metadata =
  ...
  get<DTO, Data>(DTOClass: Type<DTO>, key: K, includeParents = false): MetaValue<Data> {
    return this.getMetadata<Map<K, Data>>(DTOClass, includeParents)?.get(key);
  }

  has<DTO>(DTOClass: Type<DTO>, key: K): boolean {
    return this.getMetadata<Map<K, unknown>>(DTOClass, false)?.has(key) ?? false;
  }

  memoize<DTO, Data>(DTOClass: Type<DTO>, key: K, fn: () => Data): Data {
...
export function getPrototypeChain(Cls: Type<unknown>): Type<unknown>[] {
  const baseClass = Object.getPrototypeOf(Cls) as Type<unknown>;
  if (baseClass) {
